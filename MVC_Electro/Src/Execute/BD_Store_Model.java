...
private String pass;
	...
	public static boolean doDeleteRegsInTable(String tabla, String conditional)
	{
		try 
		{
    		senteciaSQL.executeUpdate("DELETE FROM " + tabla + " WHERE " + conditional);
		}
		catch (SQLException ex) 
		{
			System.out.println (ex.getMessage());
	...
	public static int doCountRegsInTable(String tabla, String conditional)
	{
		int count;
		
		try
		{
			cdr = senteciaSQL.executeQuery("SELECT COUNT(*) FROM " + tabla + " WHERE " + conditional);
			if(cdr.next())
			{
				count = Integer.parseInt(cdr.getString(1));
			}
			else return -1;
		}	
		catch(SQLException ex)
